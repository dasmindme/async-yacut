...
def files():
    ...
    if request.method == "POST":
        ...
        if not file_list or (len(file_list) == 1 and file_list[0].filename == ""):
        ...
        async def upload_all():
            async with aiohttp.ClientSession() as session:
                ...
                async def upload_one(file_storage):
                    ...
                    download_href = await get_download_link(session, token, disk_path)

                    return {"filename": file_storage.filename, "download_url": download_href}
        ...
        for item in uploaded:
            short_id = get_unique_short_id()
            db.session.add(URLMap(original=item["download_url"], short=short_id))
